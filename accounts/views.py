...
from accounts.models import Account, AccountExtention
...
def email_verification(request, email_arg, token_arg, first_name):
    ...
    email = EmailMultiAlternatives(
        'Mrinal Karmokar | Your Email need to be verified!',
        text_content,
...
def phone_verify_link(request, phone_arg, token_arg):
    ...
    email = request.session.get('email', default="")
    message = f'Send to: {phone_arg}, Copy this link to your browser to verify your phone no. http://localhost/accounts/verify-phone/{token_arg}'
    cust_ac = Account.objects.filter(email=email).first()
    id = cust_ac.id
    cust_acx = AccountExtention.objects.filter(account_id=id).first()
...
def send_mail_changepassword(request, email_arg, token_arg, uid_arg):
    ...
    email = EmailMultiAlternatives(
        'Mrinal Karmokar | Request for password change',
        text_content,
        settings.EMAIL_HOST_USER,
        [email_arg],
    )
